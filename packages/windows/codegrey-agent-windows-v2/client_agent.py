...
class WindowsClientAgent:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        # Initialize client-side container orchestrator
        if DOCKER_AVAILABLE:
            try:
                from .container_orchestrator import ClientContainerOrchestrator
                self.container_orchestrator = ClientContainerOrchestrator(self)
                self.logger.info("Client-side container orchestrator initialized")
            except Exception as e:
                self.logger.error(f"Failed to initialize container orchestrator: {e}")
                self.container_orchestrator = None
        else:
            self.logger.warning("Docker not available - container orchestration disabled")
            self.container_orchestrator = None
    ...
    def _execute_attack_scenario(self, command_data: Dict[str, Any]) -> Dict[str, Any]:
        """Execute attack scenario in container"""
        if not self.container_orchestrator:
            return {'success': False, 'error': 'Container orchestrator not available'}
        
        try:
            # Prepare scenario configuration
            scenario_config = {
                'attack_type': command_data.get('attack_type', 'generic'),
                'target_system': self._get_system_info(),
                'command': command_data.get('command', '/bin/bash'),
                'environment': command_data.get('environment', {}),
                'volumes': command_data.get('volumes', {})
            }
            
            # Execute attack scenario
            container_id = self.container_orchestrator.execute_attack_scenario(scenario_config)
            
            return {
                'success': True,
                'container_id': container_id,
                'message': f'Attack scenario started in container: {container_id}'
            }
            
        except Exception as e:
            return {'success': False, 'error': str(e)}
    
    def _handle_container_orchestration(self, command_data: Dict[str, Any]) -> Dict[str, Any]:
        """Handle container orchestration commands"""
        if not self.container_orchestrator:
            return {'success': False, 'error': 'Container orchestrator not available'}
        ...
        try:
            ...
            if operation == 'list':
                containers = self.container_orchestrator.get_running_containers()
                return {'success': True, 'containers': containers}
            
            elif operation == 'stop':
                container_id = command_data.get('container_id')
                self.container_orchestrator.stop_container(container_id)
                return {'success': True, 'message': f'Container {container_id} stopped'}
            
            elif operation == 'execute':
                container_id = command_data.get('container_id')
                command = command_data.get('command')
                result = self.container_orchestrator.execute_command_in_container(container_id, command)
                return result
            
            elif operation == 'logs':
                container_id = command_data.get('container_id')
                tail = command_data.get('tail', 100)
                logs = self.container_orchestrator.get_container_logs(container_id, tail)
                return {'success': True, 'logs': logs}
            
            elif operation == 'cleanup':
                self.container_orchestrator.cleanup_all_containers()
                return {'success': True, 'message': 'All containers cleaned up'}
    ...
    def _get_attack_agents(self, command_data: Dict[str, Any]) -> Dict[str, Any]:
        """Get running attack agents in API format"""
        if not self.container_orchestrator:
            # Return default agents if no container orchestrator
            return {
                'status': 'success',
                'agents': [
                    {
                        ...
                        'lastActivity': 'Container orchestrator not available',
                        'capabilities': ['Attack Planning', 'Scenario Generation', 'Red Team Operations'],
                        'platform': 'Container Agent (Unavailable)'
                    }
                ]
            }
        
        try:
            return self.container_orchestrator.get_attack_agents_api_format()
        except Exception as e:
            return {'success': False, 'error': str(e)}
    ...
    def _execute_command(self, command: Dict[str, Any]):
        ...
        try:
            ...
            elif command_type == 'attack_scenario':
                result = self._execute_attack_scenario(command_data)
            elif command_type == 'container_orchestration':
                result = self._handle_container_orchestration(command_data)
            elif command_type == 'get_attack_agents':
                result = self._get_attack_agents(command_data)
            else:
