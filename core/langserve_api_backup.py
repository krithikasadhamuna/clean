...
class SOCPlatformAPI:
    ...
    def _add_gpt_scenario_routes(self):
        ...
        try:
            ...
            @self.app.post("/api/gpt-scenarios/execute")
            ...
            async def get_gpt_scenario_suggestions(network_context: dict = None):
    ...
    def _add_custom_endpoints(self):
        ...
        @self.app.get("/api/gpt-scenarios/suggestions")
        async def get_gpt_scenario_suggestions(network_context: Dict[str, Any] = None):
            """Get GPT-generated scenario suggestions"""
            try:
                if not self.gpt_scenario_requester:
                    return {"success": False, "error": "GPT Scenario Requester not available"}
        ...
        @self.app.post("/api/gpt-scenarios/execute")
        async def execute_gpt_scenario(scenario_data: Dict[str, Any]):
            """Execute a GPT-generated scenario using the attack agent"""
            try:
                if not self.phantomstrike_ai:
                    return {"success": False, "error": "Attack agent not available"}
                ...
                if not agents:
                    return {"success": False, "error": "No client agents connected"}

                # Set target agents for scenario
                scenario_data['target_agents'] = [agent['agent_id'] for agent in agents]

                # Execute scenario using attack agent
                result = await self.phantomstrike_ai.execute_attack_scenario(scenario_data)

                logger.info(f"GPT scenario executed: {scenario_data.get('name', 'Unknown')}")
