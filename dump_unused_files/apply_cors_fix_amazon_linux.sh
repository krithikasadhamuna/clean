#!/bin/bash
# CodeGrey AI SOC Platform - Apply CORS Fix for Amazon Linux 2023
# This script applies the permanent CORS fix to your remote Amazon Linux server

echo "CodeGrey AI SOC Platform - Applying CORS Fix (Amazon Linux 2023)"
echo "================================================================"
...
if [ -f "/etc/nginx/sites-available/complete-soc" ]; then
    cp /etc/nginx/sites-available/complete-soc /etc/nginx/sites-available/complete-soc.backup.$(date +%Y%m%d_%H%M%S)
    echo "    Backup created"
else
    echo "   WARNING No existing configuration found"
fi

# Update nginx configuration with CORS fix
...
server {
    ...
    # API endpoints with CORS - matches your actual API structure
    location /api/ {
        if ($request_method = 'OPTIONS') {
            add_header 'Access-Control-Allow-Origin' $cors_origin always;
            add_header 'Access-Control-Allow-Methods' 'GET, POST, PUT, DELETE, OPTIONS, PATCH' always;
...
echo "    Nginx configuration updated with CORS fix"
...
if nginx -t; then
    ...
    if [ -f "/etc/nginx/sites-available/complete-soc.backup."* ]; then
...
# Test CORS with your actual API endpoints
echo "5. Testing CORS fix with your actual API endpoints..."
...
# Test software download endpoint
echo "   Testing software download endpoint..."
response=$(curl -s -o /dev/null -w "%{http_code}" -H "Origin: http://dev.codegrey.ai" \
    http://dev.codegrey.ai:8080/api/backend/software-download)

if [ "$response" = "200" ]; then
    echo "    Software download endpoint successful (200)"
else
    echo "   x Software download endpoint failed ($response)"
fi

echo ""
echo "================================================================"
echo "CORS Fix Applied Successfully to Amazon Linux 2023!"
echo "================================================================"
echo ""
echo " Nginx configuration updated with CORS headers"
echo " CORS preflight requests handled for all endpoints"
...
echo ""
echo "Your API endpoints now support CORS:"
...
echo "- http://dev.codegrey.ai:8080/api/backend/langgraph/attack/start"
echo "- http://dev.codegrey.ai:8080/api/backend/langgraph/detection/status"
echo ""
echo "Your frontend at http://dev.codegrey.ai should now work without CORS errors!"
echo ""
echo "To test from your frontend:"
echo "1. Remove any CORS browser extensions"
echo "2. Try making requests to your API endpoints"
echo "3. Check browser developer tools for CORS errors"
echo ""
echo "If you still have issues:"
echo "1. Check browser console for errors"
echo "2. Verify your frontend is running on http://dev.codegrey.ai"
echo "3. Check Nginx logs: sudo tail -f /var/log/nginx/error.log"
echo "4. Test with: python3 test_cors_amazon_linux.py"
