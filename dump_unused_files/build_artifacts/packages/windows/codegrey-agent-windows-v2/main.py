...
Standalone client agent for Windows endpoints
"""
...
def main():
    """Main entry point for Windows client agent"""
    print("CodeGrey AI SOC Platform - Windows Client Agent")
    print("=" * 50)
    ...
    config = config_manager.load_config()  # Ensure config file exists
    
    # Initialize and start client agent with config file path
    config_file_path = str(config_manager.config_file)
    
    try:
        print("Starting Windows client agent...")
        print(f"Server: {config.get('client', {}).get('server_endpoint', 'Not configured')}")
        print(f"Agent ID: {config.get('client', {}).get('agent_id', 'auto')}")
        print("=" * 50)
