"""
macOS-specific client agent implementation
"""
...
class MacOSClientAgent:
    """macOS-specific client agent for CodeGrey AI SOC Platform"""
    
    def __init__(self, config: Dict[str, Any]):
        self.config = config
        self.running = False
        self.server_url = config.get('server_endpoint', 'http://localhost:8080')
        self.agent_id = config.get('agent_id', f"macos-{platform.node()}")
        ...
        self.log_forwarder = MacOSLogForwarder(self.server_url, self.agent_id, config)
        self.command_executor = MacOSCommandExecutor()
        self.container_manager = MacOSContainerManager()
        ...
        self.logger.info(f"macOS client agent initialized: {self.agent_id}")
    
    def start(self):
        """Start the client agent"""
        self.running = True
        self.logger.info(f"Starting macOS client agent {self.agent_id}")
    ...
    def stop(self):
        ...
        self.logger.info("Stopping macOS client agent")
    ...
    def _get_xprotect_status(self) -> Dict[str, Any]:
        """Get XProtect (macOS built-in antivirus) status"""
        try:
            ...
            return {
                'available': True,
                'last_update': 'Unknown'  # Would need more sophisticated parsing
            }
        except:
...
class MacOSLogForwarder:
    """macOS-specific log forwarding"""
    ...
    def collect_logs(self) -> List[Dict[str, Any]]:
        ...
        # Collect system logs using log command
    ...
    def _collect_system_logs(self) -> List[Dict[str, Any]]:
        """Collect macOS system logs using log command"""
        logs = []
        
        try:
            # Use macOS log command to get recent logs
            result = subprocess.run([
            ...
            if result.returncode == 0:
                ...
                for line in lines[:50]:  # Limit to recent entries
                    if line.strip():
                        log_entry = {
                            'timestamp': datetime.now().isoformat(),
                            'source': 'macOS-System',
            ...
            self.logger.warning("System log collection timed out")
        except Exception as e:
...
class MacOSContainerManager:
    """Manage containers on macOS (Docker)"""
    ...
    def execute_container_command(self, command_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not self.container_runtime:
            return {'success': False, 'error': 'No container runtime available (Docker Desktop required)'}
