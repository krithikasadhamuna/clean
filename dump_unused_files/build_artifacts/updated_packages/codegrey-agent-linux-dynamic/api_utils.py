...
class APIUtils:
    ...
    async def get_network_topology_data(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # Get ALL registered client agents with service information (no time filter)
            cursor.execute('''
                SELECT a.id, a.hostname, a.ip_address, a.platform, a.status, a.last_heartbeat, a.agent_version,
                       GROUP_CONCAT(DISTINCT 
                           CASE 
                               WHEN le.message LIKE '%svchost.exe%' THEN 'Windows Services'
                               WHEN le.message LIKE '%powershell.exe%' THEN 'PowerShell'
                               WHEN le.message LIKE '%cmd.exe%' THEN 'Command Prompt'
                               WHEN le.message LIKE '%chrome.exe%' THEN 'Chrome Browser'
                               WHEN le.message LIKE '%firefox.exe%' THEN 'Firefox Browser'
                               WHEN le.message LIKE '%explorer.exe%' THEN 'Windows Explorer'
                               WHEN le.message LIKE '%winlogon.exe%' THEN 'Windows Logon'
                               WHEN le.message LIKE '%lsass.exe%' THEN 'Local Security Authority'
                               WHEN le.message LIKE '%csrss.exe%' THEN 'Client Server Runtime'
                               WHEN le.message LIKE '%dwm.exe%' THEN 'Desktop Window Manager'
                               WHEN le.source = 'Windows-Network' THEN 'Network Services'
                               WHEN le.source = 'Windows-Process' THEN 'Process Monitor'
                               WHEN le.source = 'Windows-System' THEN 'System Services'
                               ELSE NULL
                           END
                       ) as services
                FROM agents a
            ...
            data = []
            for agent in agents:
                agent_id, hostname, ip_address, platform, status, last_heartbeat, agent_type, services_str = agent
                ...
                if last_heartbeat:
                    try:
                        ...
                        current_status = "active" if time_diff < 300 else "inactive"
                    except:
                        current_status = "inactive"
                else:
                    current_status = "inactive"
                
                # Determine REAL physical location using advanced detection
                location_info = self._detect_real_location(ip_address, hostname)
                location = location_info.get('physicalLocation', 'Unknown')
                network_zone = location_info.get('networkZone', 'Unknown')
                ...
                data.append({
                    ...
                    "location": location,  # Real physical location
                    "networkZone": network_zone,  # Network security zone
                    "city": city,  # Actual city
                    "country": country,  # Actual country
                    "status": current_status,
                    "services": services,  # Populated from actual logs
            ...
            return {
                ...
                "metadata": {
                    ...
                    "networkZones": list(set([d["networkZone"] for d in data if d.get("networkZone")])),
                    "physicalLocations": list(set([d["location"] for d in data if d.get("location")])),
                    "cities": list(set([d["city"] for d in data if d.get("city") and d["city"] != "Unknown"])),
                    "countries": list(set([d["country"] for d in data if d.get("country") and d["country"] != "Unknown"])),
    ...
    def _detect_real_location(self, ip_address: str, hostname: str) -> Dict[str, Any]:
        ...
        try:
            ...
            location_info = await self._learn_location_from_environment(ip_address, hostname)
            ...
            return {
                ...
                'city': await self._learn_city_from_network_behavior(ip_address),
                'country': await self._detect_country_from_traffic_patterns(ip_address)
            }
    ...
    async def _learn_location_from_environment(self, ip_address: str, hostname: str) -> Dict[str, Any]:
        ...
        try:
            ...
            location_data = {
                'physicalLocation': await self._infer_location_from_network_traffic(ip_address, cursor),
                'networkZone': await self._classify_network_zone_from_behavior(ip_address, cursor),
                'city': await self._extract_city_from_dns_queries(ip_address, cursor),
                'country': await self._determine_country_from_time_patterns(ip_address, cursor)
            }
    ...
    async def _infer_location_from_network_traffic(self, ip_address: str, cursor) -> str:
    ...
    async def _classify_network_zone_from_behavior(self, ip_address: str, cursor) -> str:
    ...
    async def _extract_city_from_dns_queries(self, ip_address: str, cursor) -> str:
    ...
    async def _determine_country_from_time_patterns(self, ip_address: str, cursor) -> str:
    ...
    async def _learn_city_from_network_behavior(self, ip_address: str) -> str:
    ...
    async def _detect_country_from_traffic_patterns(self, ip_address: str) -> str:
