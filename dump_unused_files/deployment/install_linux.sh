...
if [ "$EUID" -eq 0 ]; then
    echo "WARNING:  Running as root. Consider using a regular user for security."
fi
...
sudo dnf install bcc-tools python3-bcc -y || echo "WARNING:  eBPF tools installation failed (optional)"
