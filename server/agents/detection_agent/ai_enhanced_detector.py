...
class AIEnhancedThreatDetector:
    ...
    def __init__(self):
        ...
        # Load production-ready ML models
        self._load_production_ml_models()
        ...
        self.detection_stats = {
            ...
            'stage_3_analyses': 0,
            'stage_3_skipped': 0,
            'ml_model_detections': 0
        }
        
        logger.info("AI-Enhanced Threat Detector initialized with 3-stage pipeline + 6 production-ready ML models")
    
    def _load_production_ml_models(self):
        """Load production-ready ML models from DEPLOY_READY_SOC_MODELS"""
        try:
            ...
            self.ml_manager = ml_model_manager
            
            # Get model info
            model_info = self.ml_manager.get_model_info()
            logger.info(f"Loaded {model_info['total_models']} production-ready ML models: {', '.join(model_info['loaded_models'])}")
            
        except Exception as e:
            logger.error(f"Failed to load production-ready ML models: {e}")
    ...
    async def _ai_binary_classification(self, detection_data: Dict, context: Dict) -> Dict:
        ...
        """
        try:
            message = (detection_data.get('message') or '').lower()
            source = (detection_data.get('source') or '').lower()
            command_line = (detection_data.get('command_line') or '').lower()
    ...
    def _detect_generic_log_entry(self, log_data: Dict) -> Dict:
        """ML-based detection for log entries using production-ready trained models"""
        try:
            message = log_data.get('message') or ''
            command_line = log_data.get('command_line') or ''
            source = (log_data.get('source') or '').lower()
            
            # Combine message and command line for analysis
            full_text = f"{message} {command_line}".strip()
            
            if not full_text:
                return self._heuristic_fallback_detection(log_data)
            
            # METHOD 1: Use production-ready text log anomaly detector
            ml_confidence = 0.0
            ml_threat_detected = False
            ml_threat_type = 'unknown'
            
            if self.ml_manager:
                try:
                    # Use production-ready text-based log anomaly detection (99.88% accuracy)
                    prediction, confidence = self.ml_manager.predict_text_log_anomaly(full_text)
                    
                    ml_threat_detected = bool(prediction == 1)
                    ml_confidence = float(confidence)
                    
                    if ml_threat_detected:
                        ml_threat_type = 'text_log_anomaly_detected'
                        self.detection_stats['ml_model_detections'] += 1
                    
                    logger.debug(f"Production ML Model (Text Log Anomaly): threat={ml_threat_detected}, confidence={ml_confidence:.2f}")
                    
                except Exception as ml_error:
                    logger.warning(f"Production ML model prediction failed: {ml_error}")
            
            # METHOD 3: Heuristic enhancement (combine with ML)
            heuristic_result = self._heuristic_fallback_detection(log_data)
            heuristic_confidence = heuristic_result.get('confidence_score', 0.0)
            
            # Combine ML and heuristic results
            combined_confidence = max(ml_confidence, heuristic_confidence)
            combined_threat_detected = ml_threat_detected or heuristic_result.get('threat_detected', False)
            
            # Determine final threat type
            if ml_threat_detected:
                final_threat_type = ml_threat_type
            elif heuristic_result.get('threat_detected'):
                final_threat_type = heuristic_result.get('threat_type', 'heuristic_detected')
            else:
                final_threat_type = 'clean'
            
            return {
                'threat_detected': combined_threat_detected,
                'confidence_score': combined_confidence,
                'threat_type': final_threat_type,
                'detection_method': 'ml_classifier' if ml_confidence > 0 else 'heuristics',
                'ml_confidence': ml_confidence,
                'heuristic_confidence': heuristic_confidence
            }
            
        except Exception as e:
            logger.error(f"ML log detection failed: {e}")
            return self._heuristic_fallback_detection(log_data)
    
    def _heuristic_fallback_detection(self, log_data: Dict) -> Dict:
        """Fallback heuristic-based detection when ML models fail"""
        try:
            message = (log_data.get('message') or '').lower()
            command_line = (log_data.get('command_line') or '').lower()
            source = (log_data.get('source') or '').lower()
            
            full_text = f"{message} {command_line}"
            ...
            high_risk_keywords = ['powershell -encodedcommand', 'invoke-expression', 'downloadstring', 
                                 'mimikatz', 'psexec', 'net user /add', 'vssadmin delete']
            
            for keyword in high_risk_keywords:
                if keyword in full_text:
                    threat_detected = True
                    confidence = 0.85
                    threat_type = 'high_risk_command'
                    break
            
            # Medium-confidence indicators
            if not threat_detected:
                medium_risk_keywords = ['powershell', 'cmd.exe', 'regsvr32', 'rundll32', 
                                       'wscript', 'cscript', 'mshta']
                
                for keyword in medium_risk_keywords:
                    if keyword in full_text:
                        # Check for suspicious context
                        if any(x in full_text for x in ['-enc', 'hidden', 'bypass', 'noprofile', 'javascript:']):
                            threat_detected = True
                            confidence = 0.70
            ...
            if not threat_detected:
                low_risk_keywords = ['error', 'failed', 'unauthorized', 'denied', 'attack', 'breach']
            ...
            return {
                ...
                'detection_method': 'heuristics_fallback'
            }
    ...
    def get_detection_stats(self) -> Dict:
        ...
        stats = {
            **self.detection_stats,
            ...
            'ml_detection_ratio': (self.detection_stats['ml_model_detections'] / total),
            'ai_enabled': self.ai_enabled,
            'ml_models_loaded': {
                'attack_log_classifier': self.attack_log_classifier is not None,
                'attack_log_vectorizer': self.attack_log_vectorizer is not None,
                'mitre_classifier': self.mitre_classifier is not None,
                'base_anomaly_detector': self.base_detector.anomaly_detector is not None
            }
        }
        ...
        try:
            ...
            stats['ai_analyzer_status'] = {'status': 'unknown', 'error': str(e)}
