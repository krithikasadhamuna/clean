...
class SOCPlatformAPI:
        ...
        try:
            ...
            async def agent_heartbeat(agent_id: str, heartbeat_data: dict = {}):
                ...
                try:
                    ...
                    db_manager = DatabaseManager(db_path="dev_soc_database.db", enable_elasticsearch=False, enable_influxdb=False)
            ...
            async def ingest_logs(logs_data: dict):
                ...
                try:
                    from datetime import datetime
                    from log_forwarding.server.storage.database_manager import DatabaseManager
                    from log_forwarding.shared.models import LogEntry
                    import json
                    ...
                    for log_entry in logs:
                        try:
                            ...
                            # Simple threat detection without complex analysis
                            if log_entry.get('level') in ['ERROR', 'CRITICAL', 'WARNING']:
                                threats_detected += 1
                                logger.info(f"Alert: {log_entry.get('level')} level log from {agent_id}")
    ...
    async def _analyze_threat_dynamically(self, log_entry: Dict, agent_id: str) -> Dict[str, Any]:
        """Analyze threat completely dynamically by learning from environment"""
        ...
        try:
            ...
            # Learn threat patterns from historical data
            conn = sqlite3.connect('dev_soc_database.db')
            cursor = conn.cursor()
            
            # 1. Learn from previous detections
            threat_score = await self._learn_from_historical_detections(message, source, cursor)
            
            # 2. Analyze behavioral anomalies
            behavioral_score = await self._analyze_behavioral_anomalies(log_entry, agent_id, cursor)
            
            # 3. Learn from network context
            network_score = await self._analyze_network_context(log_entry, cursor)
            
            # 4. Use ML to classify threat type
            threat_type = await self._ml_classify_threat_type(log_entry, cursor)
            
            # 5. Generate dynamic indicators
            indicators = await self._generate_dynamic_indicators(log_entry, cursor)
            
            conn.close()
            
            # Combine all scores
            final_score = max(threat_score, behavioral_score, network_score)
    ...
    async def _learn_from_historical_detections(self, message: str, source: str, cursor) -> float:
        """Learn threat patterns from historical detection results"""
        try:
            # Find similar messages that were previously detected as threats
            cursor.execute('''
                SELECT dr.confidence_score, dr.threat_type 
                FROM detection_results dr
                JOIN log_entries le ON dr.log_entry_id = le.id
                WHERE dr.threat_detected = 1 
                AND (le.message LIKE ? OR le.source = ?)
                ORDER BY dr.detected_at DESC LIMIT 10
            ''', (f'%{message[:50]}%', source))
            
            similar_threats = cursor.fetchall()
            
            if similar_threats:
                # Calculate average threat score from similar cases
                avg_score = sum(threat[0] for threat in similar_threats) / len(similar_threats)
                return min(avg_score * 1.2, 1.0)  # Boost but cap at 1.0
            
            return 0.0
            
        except Exception as e:
            logger.debug(f"Historical learning failed: {e}")
            return 0.0
    
    async def _analyze_behavioral_anomalies(self, log_entry: Dict, agent_id: str, cursor) -> float:
